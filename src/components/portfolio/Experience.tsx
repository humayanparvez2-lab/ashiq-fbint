...
export const Experience = () => {
  return (
    <section id="experience" className="bg-foreground text-background py-28 md:py-36 rounded-t-[2.5rem]">
      <div className="mx-auto max-w-6xl px-5">
        ...
        <ol className="relative md:pl-0">
          {roles.map((r, i) => (
            <li key={i} className="reveal grid md:grid-cols-12 gap-6 md:gap-10 py-10 border-t border-background/15 group">
              <div className="md:col-span-3 text-sm text-background/60 font-mono">
                {r.period}
                {r.current && (
                  <span className="ml-3 inline-flex items-center gap-1.5 rounded-full border border-background/30 px-2 py-0.5 text-[10px] uppercase tracking-wider">
              ...
              <div className="md:col-span-9">
                <div className="flex items-start justify-between gap-6">
                  <div>
                    ...
                    <div className="mt-1 text-background/70 italic font-display text-lg">
                      {r.company}
                    </div>
                  </div>
                  <ArrowUpRight className="h-6 w-6 text-background/40 transition-all group-hover:text-background group-hover:-translate-y-1 group-hover:translate-x-1" />
                </div>
                <ul className="mt-6 space-y-2.5 text-background/75 max-w-2xl">
                  {r.points.map((p) => (
